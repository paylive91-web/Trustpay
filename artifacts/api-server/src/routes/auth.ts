...
router.post("/otp/send", async (req, res) => {
  ...
  const { channel: requestedChannel } = (req.body || {}) as { channel?: string };
    const channel = requestedChannel === "whatsapp" ? "whatsapp" : "sms";
    const result = await issueOtp({ phone: String(phone), purpose, ip: clientIp(req), channel });
    if (!result.ok) {
      res.status(result.status).json({ error: result.error });
      return;
    }
    const msg = result.channel === "whatsapp"
      ? "OTP sent on WhatsApp. Please check your WhatsApp."
