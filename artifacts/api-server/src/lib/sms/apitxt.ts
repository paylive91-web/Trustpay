...
export async function sendOtpViaApitxt(phone: string, otp: string): Promise<boolean> {
  ...
  try {
    ...
    if (!res.ok || parsed?.status === "error") {
      ...
      logger.warn({ status: res.status, body: text.slice(0, 500) }, "apitxt OTP failed");
      throw new Error(`APItxt error: ${errMsg}`);
    }

    logger.info({ phone: mobile.slice(-4), requestId: parsed?.data?.request_id, cost: parsed?.data?.cost }, "apitxt OTP sent");
    return true;
  } finally {
    clearTimeout(timer);
  }
