import jwt from "jsonwebtoken";
  ...
  export async function issueOtp(opts: { phone: string; purpose: OtpPurpose; ip: string; channel?: "sms" | "whatsapp" }): Promise<{ ok: true; channel: "sms" | "whatsapp" } | { ok: false; error: string; status: number }> {
    ...
    // CRITICAL ORDERING: Send the message BEFORE writing anything to the DB.
    // If the send fails (provider 5xx, network hiccup, etc.) we don't want a
    // stale OTP row blocking retries via the 60s cooldown or burning a slot
    // from the per-hour quota.
    try {
