...
export default function AppStartupPopup() {
  ...
  const currentAnn = queue[currentIndex];
    ...
    <div className="fixed inset-0 z-[9999] flex items-center justify-center">
      ...
      {/* Dialog */}
      <div className="relative w-[min(92vw,420px)] rounded-[32px] overflow-hidden shadow-[0_32px_100px_rgba(234,88,12,0.40)] animate-in fade-in zoom-in-95 duration-300">

        {/* Glow ring */}
        <div className="absolute inset-0 rounded-[32px] ring-1 ring-inset ring-white/20 pointer-events-none z-10" />

        {/* Header */}
        <div className="relative bg-gradient-to-br from-[#c2410c] via-[#ea580c] to-[#f97316] px-5 pt-6 pb-5 overflow-hidden">
          {/* Decorative orbs */}
          <div className="absolute -top-6 -left-6 w-28 h-28 rounded-full bg-white/10 blur-2xl pointer-events-none" />
          <div className="absolute -bottom-4 -right-4 w-24 h-24 rounded-full bg-amber-300/20 blur-xl pointer-events-none" />
          {/* Shimmer line */}
          ...
          <p className="text-center text-[11px] font-semibold uppercase tracking-[0.18em] text-orange-100 mb-1">
            TrustPay
          </p>
        ...
        <div className="bg-white px-5 pt-3 pb-6">
          {queue.length > 1 && currentIndex < queue.length - 1 ? (
            <div className="flex gap-3">
              ...
              <Button
                onClick={handleNext}
                className="flex-1 h-12 rounded-2xl bg-gradient-to-r from-[#f97316] to-[#ea580c] text-white font-bold shadow-lg shadow-orange-500/30 hover:opacity-95 active:scale-[0.98] transition-all"
              >
            ...
            <Button
              onClick={handleNext}
              className="w-full h-12 rounded-2xl bg-gradient-to-r from-[#f97316] to-[#ea580c] text-white text-[16px] font-bold shadow-lg shadow-orange-500/30 hover:opacity-95 active:scale-[0.98] transition-all"
            >
              Got it
            </Button>
          )}
