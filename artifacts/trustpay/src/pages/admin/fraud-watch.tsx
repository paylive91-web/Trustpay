...
import { Snowflake, BellRing, Check, ShieldCheck, Trash2, Info } from "lucide-react";
...
export default function FraudWatch() {
  ...
  const bulkResolveUser = async (userId: number) => {
    if (!confirm(`Resolve ALL open alerts for User #${userId}?`)) return;
    setActionLoading(`bulk-${userId}`);
    try {
      await adminApi(`/admin/users/${userId}/resolve-all-alerts`, { method: "POST" });
      toast({ title: `All alerts resolved for User #${userId}` });
      invalidate(); refetch();
    } catch (e: any) {
  ...
  const markTrusted = async (userId: number, username: string, isTrusted: boolean) => {
    setActionLoading(`trust-${userId}`);
    try {
      await adminApi(`/admin/users/${userId}/trust`, { method: "POST", body: JSON.stringify({ isTrusted }) });
      toast({ title: isTrusted ? `${username} marked Trusted` : `${username} trust removed` });
      invalidate(); refetch();
    } catch (e: any) {
      toast({ title: "Failed", description: e.message, variant: "destructive" });
    } finally {
  ...
  const resetWarnings = async (userId: number) => {
    if (!confirm(`Reset fraud warning count for User #${userId} to zero?`)) return;
    setActionLoading(`warn-${userId}`);
    try {
      await adminApi(`/admin/users/${userId}/reset-fraud-warnings`, { method: "POST" });
      toast({ title: `Fraud warnings reset for User #${userId}` });
      invalidate(); refetch();
    } catch (e: any) {
      toast({ title: "Failed", description: e.message, variant: "destructive" });
    } finally {
        ...
        <Card className="border-blue-100 bg-blue-50/50">
          <CardContent className="p-3 flex items-start gap-2">
            ...
            <p className="text-xs text-blue-800">
              Alerts are auto-generated by fraud detection rules. Per-alert actions:
              <strong className="ml-1">Notify</strong> — sends an in-app message to the user.
              <strong className="ml-1">Freeze</strong> — locks the user's account immediately.
              <strong className="ml-1">Resolve</strong> — marks one alert as reviewed.
              Per-user quick actions (hover a user's card):
                      ...
                      {(grouped[sev] || []).map((a) => (
                        <div key={a.id} className="border rounded-lg p-3 flex justify-between gap-3 flex-wrap">
                          ...
                          <div className="flex gap-2 items-start flex-wrap">
                            ...
                            {a.user && !a.resolved && (
                              <Button size="sm" variant="outline" className="text-blue-700 border-blue-300" onClick={() => bulkResolveUser(a.user!.id)} disabled={actionLoading === `bulk-${a.user!.id}`}>
                                <Check className="h-3 w-3 mr-1" />Bulk Resolve
                              </Button>
                            )}
                            {a.user && (
                              <Button size="sm" variant="outline" className="text-teal-700 border-teal-300" onClick={() => markTrusted(a.user!.id, a.user!.username, !(a.user as any).isTrusted)} disabled={actionLoading === `trust-${a.user!.id}`}>
                                <ShieldCheck className="h-3 w-3 mr-1" />{(a.user as any).isTrusted ? "Untrust" : "Trust"}
                              </Button>
                            )}
                            {a.user && !a.resolved && (
                              <Button size="sm" variant="outline" className="text-amber-700 border-amber-300" onClick={() => resetWarnings(a.user!.id)} disabled={actionLoading === `warn-${a.user!.id}`}>
                                <Trash2 className="h-3 w-3 mr-1" />Reset Warns
                              </Button>
                            )}
                          </div>
                        </div>
