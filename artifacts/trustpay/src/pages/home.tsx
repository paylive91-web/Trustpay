            ...
            <div className="p-4 sm:p-5 pb-4 bg-gradient-to-r from-orange-400/10 to-rose-400/10">
              <div className="flex items-center justify-between mb-4 gap-3">
                <div>
                  ...
                  <div className="text-3xl sm:text-4xl font-bold tracking-tight bg-gradient-to-r from-orange-500 via-amber-500 to-yellow-500 bg-clip-text text-transparent">₹ {balance.toLocaleString("en-IN", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</div>
                </div>
...
// ── Daily Task Reward Card (compact banner) ──────────────────────────────────
function DailyRewardCard() {
  const { data } = useQuery({ queryKey: ["daily-reward"], queryFn: () => api("/p2p/daily-reward"), retry: false, staleTime: 60_000 });
  if (!data?.enabled || !Array.isArray(data?.tiers) || data.tiers.length === 0) return null;
  const claimed = !!data.claimed;
  const eligibleTier = data.eligibleTier;
  return (
    <Link href="/daily-reward">
      <div className="relative overflow-hidden rounded-2xl bg-gradient-to-r from-amber-500 to-orange-500 p-4 shadow-md cursor-pointer active:scale-[.98] transition-transform">
        <div className="absolute inset-0 bg-gradient-to-r from-white/0 via-white/10 to-white/0" style={{ backgroundSize: "200% 100%", animation: "shine 3s ease-in-out infinite" }} />
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-xl bg-white/20 flex items-center justify-center shrink-0">
            <Gift className="h-5 w-5 text-white" />
          </div>
          <div className="flex-1">
            <div className="text-[10px] uppercase tracking-wider font-bold text-white/70">Every Day</div>
            <div className="text-base font-black text-white leading-tight">Daily Task Reward</div>
          </div>
          {claimed ? (
            <div className="flex items-center gap-1 text-[11px] font-bold text-white/90 bg-white/20 rounded-full px-2.5 py-1">
              <CheckCircle2 className="h-3 w-3" /> Claimed
            </div>
          ) : eligibleTier ? (
            <div className="text-[11px] font-black text-orange-900 bg-yellow-300 rounded-full px-2.5 py-1 animate-pulse">
              Claim ₹{eligibleTier.reward}!
            </div>
          ) : (
...
// ── Weekly Reward Card (compact banner) ───────────────────────────────────────
function WeeklyRewardCard() {
  const { data } = useQuery({ queryKey: ["weekly-reward"], queryFn: () => api("/p2p/weekly-reward"), retry: false, staleTime: 60_000 });
  if (!data?.enabled || !Array.isArray(data?.tiers) || data.tiers.length === 0) return null;
  const claimed = !!data.claimed;
  const eligibleTier = data.eligibleTier;
  return (
    <Link href="/weekly-reward">
      <div className="relative overflow-hidden rounded-2xl bg-gradient-to-r from-violet-500 to-purple-600 p-4 shadow-md cursor-pointer active:scale-[.98] transition-transform">
        <div className="absolute inset-0 bg-gradient-to-r from-white/0 via-white/10 to-white/0" style={{ backgroundSize: "200% 100%", animation: "shine 3s ease-in-out infinite 1s" }} />
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-xl bg-white/20 flex items-center justify-center shrink-0">
            <Trophy className="h-5 w-5 text-white" />
          </div>
          <div className="flex-1">
            <div className="text-[10px] uppercase tracking-wider font-bold text-white/70">Every Week</div>
            <div className="text-base font-black text-white leading-tight">Weekly Reward</div>
          </div>
          {claimed ? (
            <div className="flex items-center gap-1 text-[11px] font-bold text-white/90 bg-white/20 rounded-full px-2.5 py-1">
              <CheckCircle2 className="h-3 w-3" /> Claimed
            ...
            <div className="text-[11px] font-black text-purple-900 bg-yellow-300 rounded-full px-2.5 py-1 animate-pulse">
              Claim ₹{eligibleTier.reward}!
            </div>
          ) : (
            <ChevronRight className="h-5 w-5 text-white/70" />
