...
export default function Orders() {
  ...
  const { data: orders, isLoading } = useGetOrders({ 
    query: { 
      queryKey: ["/api/orders", { type: filterType !== "all" ? filterType : undefined }] 
    } 
  }, {
    request: filterType !== "all" ? { type: filterType } : undefined
  } as any);

  // Filter client-side if API doesn't fully support the param yet
  ...
  const getStatusIcon = (status: string) => {
    switch (status) {
      ...
      case "pending": default: return <Clock className="w-4 h-4 text-yellow-500 mr-1" />;
    }
  };
                ...
                <CardContent className="p-4">
                  <div className="flex justify-between items-start mb-3">
                    <div className="flex items-center">
                      ...
                      <div>
                        <div className="font-semibold capitalize text-base">{order.type}</div>
                  ...
                  <div className="bg-muted/30 rounded-lg p-3 grid grid-cols-2 gap-2 text-sm">
                    ...
                    <div>
                      ...
                      <div className="font-semibold text-green-600 flex items-center">
                        + <IndianRupee className="w-3.5 h-3.5 mr-0.5 ml-1" />
                        {order.rewardAmount.toFixed(2)}
                      </div>
                    </div>
                    <div className="col-span-2 mt-1 pt-2 border-t flex justify-between items-center">
